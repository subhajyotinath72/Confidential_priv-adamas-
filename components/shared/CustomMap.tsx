...
export const CustomMap: React.FC = () => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm p-4 sm:p-6 relative text-[#103E3B]">
      
      {/* Map Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-200 mb-4">
        <div>
          <span className="text-[10px] font-bold text-[#B58A28] uppercase tracking-widest block mb-0.5">
            Campus Live Location
          </span>
          <h3 className="text-base sm:text-lg font-bold text-[#103E3B] flex items-center">
            <MapPin className="w-5 h-5 mr-2 text-[#103E3B] flex-shrink-0" />
        ...
        <a
          ...
          className="inline-flex items-center text-xs font-bold px-3.5 py-2 rounded-xl bg-slate-100 text-[#103E3B] border border-slate-200 hover:bg-slate-200 transition-colors shadow-2xs self-start sm:self-auto"
        >
      ...
      <div className="mt-4 pt-4 border-t border-slate-200 text-xs text-slate-600 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div className="flex items-center space-x-2">
          <MapPin className="w-4 h-4 text-[#B58A28] flex-shrink-0" />
          <span>Barasat–Barrackpore Road, Barbaria, P.O Jagannathpur, Kolkata – 700126, West Bengal</span>
        </div>
        <div className="flex items-center space-x-1 text-[#103E3B] font-semibold">
