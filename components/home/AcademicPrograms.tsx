...
export const AcademicPrograms: React.FC = () => {
  ...
  const btech = ACADEMIC_PROGRAMS[0];
              ...
              return (
                <button
                  ...
                  className={`relative px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all cursor-pointer ${
                    isSelected
                      ? "text-white bg-[#103E3B] shadow-md"
                      : "text-slate-700 bg-white/80 hover:bg-white border border-slate-200"
                  }`}
                >
                  {labels[tabKey]}
                  {isSelected && (
          ...
          >
            <div className="glass-card p-6 sm:p-8 rounded-2xl h-full space-y-6 flex flex-col justify-between">
                ...
                <div className="space-y-2 pt-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {active.tracks.map((track, idx) => (
                      <div
                        key={idx}
                        className="flex items-center space-x-2 text-xs text-slate-700 bg-white/85 p-2.5 rounded-lg border border-slate-200/80 backdrop-blur-sm hover:border-[#103E3B]/40 transition-colors"
                      >
              ...
              <div className="pt-4 border-t border-slate-200/80 flex flex-wrap items-center gap-3">
                ...
                <Link
                  href="/programs"
                  className="w-full sm:w-auto inline-flex items-center justify-center px-5 py-3 rounded-md text-xs font-bold uppercase tracking-wider text-[#103E3B] bg-[#EFECE6] border border-[#D5D0C5] hover:bg-[#E5E0D5] transition-all"
                >
            ...
            <div className="glass-beige p-6 rounded-2xl shadow-sm flex flex-col justify-between h-full space-y-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-emerald-100 text-[9px] font-bold text-emerald-800 uppercase">
                ...
                <div className="space-y-3">
                  <div className="bg-white p-3.5 rounded-xl border border-slate-200/80 hover:border-amber-400/60 transition-colors shadow-xs">
                    <div className="flex items-center justify-between">
                  ...
                  <div className="bg-white p-3.5 rounded-xl border border-slate-200/80 hover:border-amber-400/60 transition-colors shadow-xs">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-bold text-teal-600 bg-teal-50 px-2 py-0.5 rounded">Rolling</span>
                    </div>
                    ...
                    {SPECIALIZATION_CHIPS.map((chip, idx) => {
                      ...
                      return (
                        <span
                          key={idx}
                          title={chip.desc}
                          className="inline-flex items-center px-2 py-1 bg-white text-[10px] font-semibold text-slate-700 border border-slate-200 rounded-md shadow-xs hover:border-[#103E3B] hover:text-[#103E3B] transition-colors cursor-help"
                        >
                          <Icon className="w-2.5 h-2.5 mr-1 text-amber-600" />
                          {chip.name}
                        </span>
                      );
                    })}
              ...
              <div>
                <a
                  ...
                  className="shine-sweep w-full inline-flex items-center justify-center px-6 py-3 rounded-md text-xs font-bold uppercase tracking-wider text-[#103E3B] bg-white border border-slate-300 hover:bg-slate-50 transition-all shadow-xs"
                >
