          ...
          <div className="overflow-x-auto pb-4 scrollbar-thin">
            <div className="flex w-max space-x-6 animate-horizontal-ticker py-2">
              {[1, 2].flatMap((iteration) => [
                ...
                <div
                  key={index}
                  className="w-[300px] sm:w-[360px] flex-shrink-0 glass-dark p-6 rounded-2xl space-y-4"
                >
                  ...
                  {card.recruiters && (
                    <div className="flex flex-wrap gap-2 pt-1">
                      {card.recruiters.map((r, i) => (
                        <span key={i} className="px-2.5 py-1 bg-white/10 text-[10px] font-bold text-[#eaf6f6] border border-white/20 rounded">
                          {r}
                        </span>
                      ))}
                    </div>
